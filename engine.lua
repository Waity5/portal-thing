...
function onTick()
	...
	if loaded then
		...
		for i,pegPoint in ipairsVar(pegPoints) do
			...
			if totalDistance > 0.0001 then
				...
				applyInstantMovement(object1,crPoint1,mul3(totalDisplacement,-1/movementFromPushing))
				applyInstantMovement(object2,crPoint1,mul3(totalDisplacement,1/movementFromPushing))
			end
			...
			if totalSpeed > 0.0001 then
				...
				applyForce(object1,crPoint1,mul3(totalVelocity,-1/movementFromPushing))
				applyForce(object2,crPoint1,mul3(totalVelocity,1/movementFromPushing))
			end
		...
		for i,spring in ipairsVar(springs) do
			...
			if distance > 0.0001 then
				...
				applyForce(object1,crPoint1,mul3(force,-1))
				applyForce(object2,crPoint1,force)
				
				force = mul3(displacementNormalised,((spring[8] or distance) - distance) * spring[7])
				applyForce(object1,crPoint1,mul3(force,-1))
				applyForce(object2,crPoint1,force)
			end
