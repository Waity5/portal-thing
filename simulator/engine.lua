...
while C~=0 do
C,O,cf,cK=F(P[5][aS])_,ar=E[cf],E[cK]
...
function aV(b,at,ax)by=h(at,b[1])
b[5]=D(b[5],g(j(by,ax),b[11]))b[2]=D(b[2],g(ax,b[10]))end
...
a,c,v,ai=F(z)
if ai then
K=h(c,a)al=h(v,a)cj=h(ai,a)A=g(a,-1)
aa=j(K,al)ck=j(al,cj)ci=j(cj,K)
if l(aa,A)>0 then
z={a,c,v}k=aa
...
au={u[1],u[2],e}au[4]=bl(bw(au[1],au[2],au[3]))
...
elseif v then
K=h(c,a)al=h(v,a)A=g(a,-1)
aa=j(K,al)
if l(j(aa,al),A)>0 then
...
as={}
for W=1,#T do
b=T[W]b[4]=bC(b[4],g(b[5],-bu))b[1]=D(b[1],g(b[2],bu))b[2]=D(b[2],g(b[3],bu))b[3]=g(b[12],1)


b[16]=cr(b[4])
for f,J in ag(b[9])do
for V,e in ag(J)do
...
a_=D(j(n[5],h(p,n[1])),n[2])aZ=D(j(r[5],h(p,r[1])),r[2])aL=h(a_,aZ)
aK=l(y[1],aL)if aK>0 then
...
bX=aT(n,p,y[1])+aT(r,p,y[1])

cI=aK
bj=cI/bX

aV(n,p,g(y[1],-bj))aV(r,p,g(y[1],bj))
...
aK=l(y[1],aL)
bJ=h(aL,g(y[1],aK))
bK=bs(bJ,{0,0,0})if bK>.001 then
aJ=bl(bJ)cA=aT(n,p,aJ)+aT(r,p,aJ)bZ=aR(bK/cA,bj*.5)
aV(n,p,g(aJ,-bZ))aV(r,p,g(aJ,bZ))end
...
for d=1,#b[7]do
e=b[7][d]e[2]=bp(e[1],b[16])e[2]=D(e[2],b[1])
end
...
for d=1,#b[8]do
i=b[8][d]
i[8]=bw(b[7][i[1]][2],b[7][i[2]][2],b[7][i[3]][2])end
end
end
...
table.sort(as,function(a,c)return a[5]>c[5]end)
...
for d=1,#b[7]do
e=b[7][d]e[3]=h(e[2],dp)
e[4]=bp(e[3],dn)br=e[3]e[7]=aN(br[1]^2+br[2]^2+br[3]^2)
e[5]={e[4][1]*be/e[4][3],-e[4][2]*be/e[4][3]}e[6]=e[4][3]>0 and 1 or 0
...
if am==2 then
bg=aW(aQ(aM(ae,af),1000),ae)bf=aW(aQ(aM(ad,af),1000),ad)




t={ad,ae,bg,bf}else
bg=aW(aQ(aM(af,ae),1000),ae)bf=aW(aQ(aM(af,ad),1000),ad)
...
if am>=1 then
m=t[1]q=t[2]s=t[3]
bH=aR(m[1],q[1],s[1])bG=av(m[1],q[1],s[1])bF=aR(m[2],q[2],s[2])bI=av(m[2],q[2],s[2])
if(bG>cO and bH<cN)and(bI>cM and bF<cP)then
